...
namespace SapsanApp.Controllers.v1._0
{
    ...
    public class SourcesController : ControllerBase
    {
        ...
        private IGenericRepository<Source, int> _rep;
        public SourcesController(
            IGenericRepository<Source, int> rep,
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                _context.Logs.Add(new Log()
                {
                    ...
                    Description = $"Getting a list of sources by {User.Identity.Name}",
                    Type = "Get"
                });
        ...
        {
            if (_context.Sources.FirstOrDefault(x => x.Id == id) == null)
                return NotFound();
            ...
            try
            {
                ...
                {
                    UserId = _context.Users.AsNoTracking().FirstOrDefault(x => x.Email == User.Identity.Name).Id,
                    Description = $"Getting a source by ID by {User.Identity.Name}",
                    Type = "Get"
                });
        ...
        [EnableCors]
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Source model)
        {
            if (model == null)
            ...
            try
            {
                ...
                {
                    UserId = _context.Users.AsNoTracking().FirstOrDefault(x => x.Email == User.Identity.Name).Id,
                    Description = $"Posted a sources data by {User.Identity.Name}",
                    Type = "Post"
                });
        ...
        [EnableCors]
        [HttpPut]
        public async Task<IActionResult> Put([FromBody] Source model)
        {
            if (model == null)
                return BadRequest(new { errorText = "Invalid input data!" });
            ...
            try
            {
                ...
                {
                    UserId = _context.Users.AsNoTracking().FirstOrDefault(x => x.Email == User.Identity.Name).Id,
                    Description = $"Edited a source data by {User.Identity.Name}",
                    Type = "Put"
                });
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                {
                    UserId = _context.Users.AsNoTracking().FirstOrDefault(x => x.Email == User.Identity.Name).Id,
                    Description = $"Removed a sources data by {User.Identity.Name}",
                    Type = "Delete"
                });
